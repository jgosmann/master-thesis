...
\mat I$ as a shorter notation. By conditioning on the observations the 
...
\begin{figure}
    ...
    \caption[Gaussian process example]{Example of a one-dimensional Gaussian 
        ...
        points $X$: \subref{fig:ex-gp} shows the mean $\bar{\vc f_*}$ and 
        predictive standard deviation $\sigma(X_*)$; \subref{fig:ex-gp-sample} 
        shows three functions sampled from the process.}\label{fig:ex-gp-main}
\end{figure}
...
the noisy data.  Half-integer values are used as the kernel functions will 
become quite simple:
\begin{align}
...
worse than with stationary kernel agnostic to the structure. Especially when 
...
respect to what should be considered a good model. The root mean square erroris 
...
expected benefit for choosing $\vc x$ as next training sample.  Hence, the 
optimal choice is $\argmax_{\vc x} u(\vc x)$.  Equivalently, it is possible to 
use the negative of a loss function $u(\vc x) = - \lambda(\vc x)$. In the next 
section different possible acquisition functions will be discussed in the 
