...
Equations~\ref{eqn:meanpred} and~\ref{eqn:covpred} after $t$ updates by $-\mat 
C_t = \tilde{\mat K}_t^{-1} + \mat R_{C,t}$.\footnote{The original paper 
    expresses this a bit different. The equivalence of both notations is shown 
    in Appendix~\ref{sec:sparse-gp-apdx}.} Normal (full) online updates are 
performed as discussed in the previous section by updating $\tilde{\mat K}_t$ 
respectively the Cholesky factor $\mat L_t$. If, however, the ``novelty''
\begin{equation}
    \tilde{\sigma}^2_{t+1} = k(\vc x_*, \vc x_*) + \sigma\ped{n}^2 
    \mat{\mathrm{I}} - K\!\del{\{\vc x_*\}, X_t} \tilde{\mat K}_t^{-1} 
    K\!\del{X_t, \{\vc x_*\}}
\end{equation}
of a new input $\vc x_*$ is below a threshold $\epsilon\ped{tol}$, reduced 
update will be performed. The reduced update leaves the size of $\mat C_t$ 
unchanged, while adjusting $\mat R_{C,t}$ to improve the fit of the Gaussian 
process.
...
vector. Given the inverse Cholesky factor $\mat L^{-1}$ the inverse downdated 
covariance matrix can be easily and efficiently obtained (see 
Appendix~\ref{sec:downdating}).  Unfortunately, downdating based on Cholesky 
factorization is known to be numerical unstable \parencite{Bjorck:1994dz}, 
especially when the correlation of the training inputs is high. Using other 
factorizations like the QR factorization more stable downdating algorithm can be 
obtained.  However, this comes at the cost of the initial factorization being 
numerical more unstable.  Also, it would be possible to directly downdate 
...
It is desirable to have differentiable acquisition functions to be able to used 
